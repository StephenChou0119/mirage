...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--is-qwen2", action="store_true", help="Is Qwen2 model"
    )
    ...
    if args.use_mirage:
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            if args.is_qwen2:
                bias_q_torch = layer.self_attn.q_proj.bias
                bias_k_torch = layer.self_attn.k_proj.bias
                bias_v_torch = layer.self_attn.v_proj.bias
                use_bias = 1
            else:
                bias_qkv = None
                use_bias = 0
                bias_q_torch = torch.zeros([layer.self_attn.q_proj.out_features], device=torch.device("cuda:0"))
                bias_k_torch = torch.zeros([layer.self_attn.k_proj.out_features], device=torch.device("cuda:0"))
                bias_v_torch = torch.zeros([layer.self_attn.v_proj.out_features], device=torch.device("cuda:0"))
            bias_q = mpk.attach_input(
                torch_tensor=bias_q_torch, name=f"layer_{i}_q_bias"
            )
            bias_k = mpk.attach_input(
                torch_tensor=bias_k_torch, name=f"layer_{i}_k_bias"
            )
            bias_v = mpk.attach_input(
                torch_tensor=bias_v_torch, name=f"layer_{i}_v_bias"
            )
            bias_qkv = mpk.fuse_tensors(
                inputs=[bias_q, bias_k, bias_v],
                fused_dim=0,
                num_groups=model.config.num_key_value_heads // world_size,
                name=f"layer_{i}_qkv_bias",
            )
            ...
            # add attention
            if args.is_qwen2:
                q_norm_torch = torch.zeros([layer.self_attn.q_proj.out_features], device=torch.device("cuda:0"))
                k_norm_torch = torch.zeros([layer.self_attn.k_proj.out_features], device=torch.device("cuda:0"))
                qk_norm = 0
            else:
                q_norm_torch = layer.self_attn.q_norm.weight
                k_norm_torch = layer.self_attn.k_norm.weight
                qk_norm = 1
            w_q_norm = mpk.attach_input(
                torch_tensor=q_norm_torch, name=f"layer_{i}_q_norm"
            )
            w_k_norm = mpk.attach_input(
                torch_tensor=k_norm_torch, name=f"layer_{i}_k_norm"
            )
            ...
            fake_bias = torch.zeros([layer.mlp.gate_proj.out_features], device=torch.device("cuda:0"))
            w_bias = mpk.attach_input(
                torch_tensor=fake_bias, name=f"layer_{i}_fake_bias"
            )
            mpk.rmsnorm_linear_layer(
                input=x,
                weight_norm=w_norm,
        ...
        fake_bias = torch.zeros([lm_head_weight.shape[1]], device=torch.device("cuda:0"))
        w_bias = mpk.attach_input(
                torch_tensor=fake_bias, name=f"lm_head_bias"
            )
